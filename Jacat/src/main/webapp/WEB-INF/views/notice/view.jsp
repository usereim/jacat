...
<body>
	<div>게시글번호 : ${noticeboard.boardNum}</div>
	<div>제목 : ${noticeboard.title}</div>
	<div>본문 : ${noticeboard.content}</div>
	<div>작성자 : ${noticeboard.user.nick}</div>
	<div>작성일 : ${noticeboard.wDate}</div>
	<div>
	첨부파일 :
	...
	<c:if test="${sessionScope.user.id == noticeboard.usersId}">
		<input type="button" value="수정" onclick="moveModifyPage(${noticeboard.boardNum})">
		<form 
			action="<c:url value="/notice/delete/${noticeboard.boardNum}" />" 
			method="post">
